...
function draw() {
  ...
  snake.forEach((p, i) => {
    ...
    ctx.beginPath();
    ctx.roundRect(p.x + 1, p.y + 1, box - 2, box - 2, 5);
    ctx.fill();
  });
...
function gameOver() {
  ...
  document.getElementById("finalScoreMsg").innerText = `SKOR: ${score} PTS`;
}
function saveScore(n, s) {
  let l = JSON.parse(localStorage.getItem("snk") || "[]");
  l.push({ n, s });
  l.sort((a, b) => b.s - a.s);
  localStorage.setItem("snk", JSON.stringify(l.slice(0, 3)));
}
function loadLeaderboard() {
  let l = JSON.parse(localStorage.getItem("snk") || "[]");
  document.getElementById("leaderList").innerHTML = l.map((x) => `<div>${x.n}: ${x.s}</div>`).join("") || "NO DATA";
}
